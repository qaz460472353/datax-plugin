...
public class ESWriter extends Writer {
    ...
    public static class Task extends Writer.Task {
        ...
        public void startWrite(RecordReceiver recordReceiver) {
            ...
            // 新增字段，是否开启同步任务、同步目标type
            boolean isUpdate = Key.getIsUpdate(conf);
            String targetIndex = Key.getTargetIndex(conf);
            String targetType = Key.getTargetType(conf);
            String targetField = Key.getTargetField(conf);
            if (isUpdate) {
                insertAndUpdate(targetIndex,targetType,targetField);
            }
        ...
        public void insertAndUpdate(String targetIndex,String targetType, String targetField) {
            // 遍历数据，查找目标type中的数据，更新或插入
            SearchSourceBuilder searchSourceBuilder = new SearchSourceBuilder();
            searchSourceBuilder.query(QueryBuilders.matchAllQuery());
            //多条件查询
            BoolQueryBuilder boolBuiler = QueryBuilders.boolQuery();
            try {
                List<SearchResult.Hit<JSONObject, Void>> hits = esClient.search(index, type, searchSourceBuilder.toString());
                List<JSONObject> list = new ArrayList<>();
                for (SearchResult.Hit<JSONObject, Void> hit : hits) {
                    JSONObject source = hit.source;
                    list.add(source);
                    String value = source.get(targetField).toString();
                    searchSourceBuilder.query(boolBuiler.filter(QueryBuilders.termQuery(targetField, value)));
                    List<SearchResult.Hit<JSONObject, Void>> oldhits = esClient.search(targetIndex, targetType, searchSourceBuilder.toString());
                    if (oldhits.size() > 0) {
                        String oldId = oldhits.get(0).id;
                        String param = "{ \"doc\" : { \"GXZT\" : \"02\" }}";
                        // 更新 oldResult
                        boolean update = esClient.update(targetIndex, targetType, oldId, param);
                    }
                }
                // 将中间库数据插入目标库
                boolean b = esClient.batchInsert(targetIndex, targetType, list);
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
