...
//    }
	...
	public static void doPretreatment(Configuration originalConfig, DataBaseType dataBaseType) {
		...
		simplifyConf(originalConfig,dataBaseType,false);
		if(dataBaseType!=DataBaseType.Sqlite) {
			dealColumnConf(originalConfig);
		}else {
			dealSqliteColumnConf(originalConfig);
		}
	 ...
	 * @param dataBaseType
	 * @param creatNewFile 针对Sqlite 是否需要产生新数据库
	 */
	public static void simplifyConf(Configuration originalConfig,DataBaseType dataBaseType,boolean creatNewFile) {
		...
		for (int i = 0, len = connections.size(); i < len; i++) {
			...
			if (StringUtils.isBlank(jdbcUrl)&&dataBaseType!=DataBaseType.Sqlite) {
				throw DataXException.asDataXException(DBUtilErrorCode.REQUIRED_VALUE, "您未配置的写入数据库表的 jdbcUrl.");
			}else if(dataBaseType==DataBaseType.Sqlite) {
				if(creatNewFile) {
					String newfile=originalConfig.getNecessaryValue(Key.FILE_NAME,DBUtilErrorCode.REQUIRED_VALUE);
					newfile=newfile+"_"+new Date().getTime();
					originalConfig.set(Key.FILE_NAME, newfile);
				}
			   jdbcUrl="jdbc:sqlite:"+originalConfig.getNecessaryValue(Key.Path,DBUtilErrorCode.REQUIRED_VALUE)+File.separator+originalConfig.getNecessaryValue(Key.FILE_NAME,DBUtilErrorCode.REQUIRED_VALUE);
	...
	public static void dealColumnConf(Configuration originalConfig, ConnectionFactory connectionFactory,
			String oneTable) {
		...
		if (null == userConfiguredColumns || userConfiguredColumns.isEmpty()) {
			...
			if (isPreCheck) {
				allColumns = DBUtil.getTableColumnsByConn(DATABASE_TYPE, connectionFactory.getConnecttionWithoutRetry(),
						oneTable, connectionFactory.getConnectionInfo());
			} else {
			...
			if (1 == userConfiguredColumns.size() && "*".equals(userConfiguredColumns.get(0))) {
				...
				DBUtil.getColumnMetaData(connectionFactory.getConnecttion(), oneTable,
						StringUtils.join(userConfiguredColumns, ","));
			}
		}
	}
	
	public static void dealSqliteColumnConf(Configuration originalConfig) {
		List userConfiguredColumns = originalConfig.getList(Key.COPY_COLUMN);
		if(userConfiguredColumns!=null&&!userConfiguredColumns.isEmpty()) {
			List<String> column=new ArrayList<String>();
			List<String> allColumns=new ArrayList<String>();
			for (Object col : userConfiguredColumns) {
                JSONObject jo = JSONObject.parseObject(col.toString());
                String colName = jo.getString("name");
                String colTypeStr = jo.getString("type");
                allColumns.add(colName);
                column.add(colName+" "+colTypeStr);
			}
			originalConfig.set(Key.COLUMN, allColumns);
			SqliteCommon common=new SqliteCommon(originalConfig);
			common.creatSqliteTable(StringUtils.join(column,","));
		}
	}
	...
	public static void dealColumnConf(Configuration originalConfig) {
		...
		JdbcConnectionFactory jdbcConnectionFactory = new JdbcConnectionFactory(DATABASE_TYPE, jdbcUrl, username,
				password);
	...
	public static void dealWriteMode(Configuration originalConfig, DataBaseType dataBaseType) {
		...
		String jdbcUrl = originalConfig
				.getString(String.format("%s[0].%s", Constant.CONN_MARK, Key.JDBC_URL, String.class));
